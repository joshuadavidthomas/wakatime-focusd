...
- `SIGHUP` config reload — send `kill -HUP $(pidof wakatime-focusd)` or `systemctl --user reload wakatime-focusd` to reload the configuration file without restarting the daemon. All settings except `backend` are applied immediately; changing the backend requires a full restart. The generated systemd unit now includes an `ExecReload` directive so `systemctl --user reload` works out of the box.
