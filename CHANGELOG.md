...
- **Removed `async-trait` dependency** — `FocusSource` and `HeartbeatSender` traits now use explicit `BoxFuture` return types instead of the `#[async_trait]` proc macro. This removes one dependency and its hidden `Box<dyn Future>` allocations generated by the macro. No changes to runtime behavior.
